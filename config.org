...
* Introduction
Zmacs is a very simple and basic configuration file for Emacs. From here you can edit, remove, or add many aspects of Emacs you want. I encourage you to do so, because Emacs works best when it's customized to your own liking!
* Why would I use this?
#+BEGIN_QUOTE
What's the difference between a villain and a super villain? Presentation!
#+END_QUOTE
It is a very basic config which means you can easily add, edit or remove things as you like. You can use this as a building block when making your OWN Emacs config!
...
#+END_QUOTE
As opposed to many other Emacs distributions, Zmacs has ZERO customization layers which means you can just jump in, look at the config file and start editing away! After all, Emacs works best when it's customized to your own liking!
* Quick install
#+BEGIN_SRC 
git clone https://github.com/Jack-O-Brian/Zmacs ~/.emacs.d
#+END_SRC
...
*** Description
The frontend of Zmacs; without this there'd be no Marisa in your Emacs startup screen
*** Code
#+BEGIN_SRC emacs-lisp
  (use-package dashboard
    ...
    :preface
    (defun update-config ()
      "Update Zmacs to the latest version."
      (interactive)
      (let ((dir (expand-file-name user-emacs-directory)))
        (if (file-exists-p dir)
            (progn
              (message "Zmacs is updating!")
              (cd dir)
    ...
    (setq dashboard-banner-logo-title "Z M A C S")
    ...
    (setq dashboard-navigator-buttons
          `(;; line1
            ((,nil
              "Zmacs on github"
              "Open Zmacs' github page on your browser"
              (lambda (&rest _) (browse-url "https://github.com/Jack-O-Brian/zmacs"))
              'default)
             (nil
              "Zmacs crash course"
              "Open Zmacs' introduction to Emacs"
              (lambda (&rest _) (find-file "~/.emacs.d/Zeat.org"))
              'default)
             (nil
              "Update Zmacs"
              "Get the latest Zmacs update. Check out the github commits for changes!"
            ...
            ((,nil
             ...
             (nil
              "Open config.org"
              "Open Zmacs' configuration file for easy editing"
...
*** Notes
If you pay close attention to the code in dashboard, you'll  notice that it uses custom functions defined under the :preface use-package block. I wrote all of those functions by looking at other people's Emacs distributions (Mainly [[https://github.com/seagle0128/.emacs.d][Centaur Emacs]]) and then experimenting and adapting them to Zmacs. If you dig around, you'll find the same things I did - maybe even more!
*** Historical
...
*** Description
Vim keybindings in Emacs. Please note that Zmacs has NO other evil-mode compatibility packages because I like to KISS. This might change in the future
*** Code
#+BEGIN_SRC emacs-lisp
  (use-package evil
    :ensure t
